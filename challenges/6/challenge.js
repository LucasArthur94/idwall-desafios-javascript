...
// https://stackoverflow.com/a/44014849/6726176
const dropArrayRepetitions = (array) => Array.from(new Set(array.map(JSON.stringify)), JSON.parse)

const getCombinations = (subset, set, target) => {
  const result = subset.reduce((acc, el) => el + acc, 0)

  if (result === target) return [subset.sort((a, b) => a - b)]
  else if (result > target) return []

  return set.reduce(
    (newSubsets, element) =>
      dropArrayRepetitions([...newSubsets, ...getCombinations([...subset, element], set, target)]),
    [],
  )
}
...
const combinate = (set, target) => {
  return set.reduce(
    (combinations, value) =>
      dropArrayRepetitions([...combinations, ...getCombinations([value], set, target)]),
    [],
  )
}
